...
const BookingDetails: React.FC<BookingDetailsProps> = ({
      ...
      days.map((day) => {
        if (day.id === dayId) {
          ...
          if (
            ...
            const newStartTime =
              field === "startTime" ? value : activity.startTime;
  ...
  const handleEditTimeline = () => {
    ...
    const groupedActivities = timelineActivities.reduce(
      (acc, activity) => {
    ...
    const newDays = Object.entries(groupedActivities).map(
      ...
      <div className="p-4 flex flex-col items-center justify-center h-48">
        ...
        {userRole === "organizer" ? (
          <p className="text-base text-center text-blue-600">
            The event proposal is still awaiting customer's acceptance. Once
            they confirm, you can create an event timeline here.
          </p>
        ) : userRole === "individual" ? (
          <p className="text-base text-center">
            Your proposal is still under review. Once you accept it, the
            organizer can create an event timeline.
          </p>
        ) : (
          <p className="text-base text-center">
            This proposal is still under review. Once accepted, a timeline will
            be available.
          </p>
        )}
      </div>
          ...
          <div className="p-4">
            ...
            {timeline.map((item) => (
              <div key={item.id} className="flex mb-2">
                ...
                <div className="w-2/3 bg-blue-50 p-2 rounded">
                  {item.activity}
                </div>
              </div>
            ))}
