...
import RSVPCard, { RSVPCardProps } from "./Elements/rsvp-cards";
...
const RSVP: React.FC = () => {
  ...
  const handleViewTracker = (index: number) => {
  ...
  const renderPagination = () => {
    ...
    const getPageRange = () => {
      if (totalPages <= visiblePages) return pageNumbers;
      if (currentPage <= middle + 1) return pageNumbers.slice(0, visiblePages);
      if (currentPage >= totalPages - middle) return pageNumbers.slice(totalPages - visiblePages);
  ...
  return (
    <div className="flex min-h-screen">
      <div className="flex flex-1 flex-col justify-between transition-all duration-300" style={{ marginLeft: "16rem" }}>
        {viewingTracker && selectedEventIndex !== null ? (
          <RSVPTracking
            onBackClick={handleBackClick}
            rsvpCreated={mockEvents[selectedEventIndex].rsvpCreated}
          />
        ) : (
          <>
                ...
                <div className="grid grid-cols-3 gap-4">
                  {currentEvents.map((event, index) => (
                    <RSVPCard
                      key={index}
                      {...event}
                      onViewTracker={() => handleViewTracker(startIndex + index)}
                    />
                  ))}
                </div>
              ) : (
                <div className="flex flex-col items-center justify-center text-center mt-24">
                  <img src={emptyBox} alt="No Events" className="w-48 h-48 mb-8" />
